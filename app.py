    ...
    with st.container():
        ...
        if max_shares > 0:
            ...
            <div class="calc-result">
                ...
                <div class="calc-res-val txt-up-vip">{max_shares:.2f} 股</div>
            ...
            with tab_analysis:
                ...
                with c1:
                    ...
                    if not df_intra.empty:
                        ...
                        # 冬令固定刻度
                        tick_vals = [
                            pd.Timestamp.combine(base_date, t_p_start).tz_localize('Asia/Taipei'),
                            pd.Timestamp.combine(base_date, t_open).tz_localize('Asia/Taipei'),
                            pd.Timestamp.combine(base_date + timedelta(days=1), t_close).tz_localize('Asia/Taipei'),
                            pd.Timestamp.combine(base_date + timedelta(days=1), t_p_end).tz_localize('Asia/Taipei')
                        ]
                        ...
                        fig_spark.update_layout(
                            ...
                            xaxis=dict(
                                visible=True, tickmode='array', tickvals=tick_vals, ticktext=tick_texts,
                                tickfont=dict(size=10, color='#000000'), showgrid=False
                            ),
