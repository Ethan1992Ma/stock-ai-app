...
def render_inventory_tab(current_close_price, quote_type):
    ...
    SEC_FEE_RATE = 0.0000278
    if quote_type == 'ETF': BUY_FIXED_FEE, BUY_RATE_FEE, SELL_FIXED_FEE, SELL_RATE_FEE = 3.0, 0.0, 3.0, SEC_FEE_RATE
    else: BUY_FIXED_FEE, BUY_RATE_FEE, SELL_FIXED_FEE, SELL_RATE_FEE = 0.0, 0.001, 0.0, 0.001 + SEC_FEE_RATE
    st.caption(f"{'💡 ETF: 固定 $3 USD' if quote_type=='ETF' else '💡 股票: 0.1%'}")
    ...
    try:
        ...
        if not df.empty and len(df) > 200:
            ...
            if strategy_mode == "🤖 自動判別 (Auto)":
                ...
                if mcap > 200_000_000_000: strat_fast, strat_slow, strat_desc = 10, 20, "🐘 巨頭穩健"
                else: strat_fast, strat_slow, strat_desc = 5, 10, "🚀 小型飆股"
            ...
            with tab_analysis:
                ...
                with c1:
                    ...
                    if not df_intra.empty:
                        ...
                        try:
                            ...
                            df_intra.index = pd.to_datetime(df_intra.index).tz_localize('UTC').tz_convert(tz)
                            df_intra_tz = df_intra

                        # [Step 2] 定義時間 (TW)
                        open_time_dt = time(22, 30)
                        close_time_dt = time(5, 0)
                        ...
                        # [Step 3] 篩選盤中數據
                        # 邏輯: 22:30 <= t <= 23:59 或 00:00 <= t <= 05:00
                        mask_reg = (df_intra_tz.index.time >= open_time_dt) | (df_intra_tz.index.time <= close_time_dt)
                        ...
                        if not df_regular.empty:
                            ...
                            fill_color = f"rgba({('5, 154, 129' if day_close_reg >= day_open_reg else '242, 54, 69')}, 0.15)"
                        ...
                        # [Step 4] 時間軸設定 (強制鎖定4個點)
                        # 抓取資料日期的基準日
                        ref_dt = df_intra_tz.index[0]
                        if ref_dt.hour >= 12: # 下午盤 (17:00開始), 基準日為當天
                            base_date = ref_dt.date()
                        else: # 凌晨盤 (05:00結束), 基準日為昨天
                            base_date = ref_dt.date() - timedelta(days=1)
                            
                        # 建構 Tick Timestamp
                        try:
                            t1 = pd.Timestamp(f"{base_date} 17:00").tz_localize(tz)
                            t2 = pd.Timestamp(f"{base_date} 22:30").tz_localize(tz)
                            t3 = pd.Timestamp(f"{base_date + timedelta(days=1)} 05:00").tz_localize(tz)
                            t4 = pd.Timestamp(f"{base_date + timedelta(days=1)} 09:00").tz_localize(tz)
                            tick_vals = [t1, t2, t3, t4]
                        except:
                            tick_vals = [] # Fallback
                        ...
                        # [修正2: H/L 顏色] 改用行內樣式強制指定顏色，確保不被 CSS 覆蓋
                        h_col = COLOR_UP if day_high_pct >= 0 else COLOR_DOWN
                        l_col = COLOR_UP if day_low_pct >= 0 else COLOR_DOWN
                        
                        price_html += f"""<div class="spark-scale">
                            <div style="color: {h_col} !important; font-weight: bold;">H: {day_high_pct:+.1f}%</div>
                            <div style="margin-top:25px; color: {l_col} !important; font-weight: bold;">L: {day_low_pct:+.1f}%</div>
                        </div></div>"""
                ...
                with k1: st.markdown(f"""<div class="metric-card"><div class="metric-title">趨勢訊號</div><div class="metric-value" style="font-size:1.3rem;">{trend_msg}</div><div><span class="status-badge {trend_bg}">MA{strat_fast} vs MA{strat_slow}</span></div><div class="metric-sub">多空不明，建議觀望</div></div>""", unsafe_allow_html=True)
                with k2: st.markdown(f"""<div class="metric-card"><div class="metric-title">量能判讀</div><div class="metric-value" style="font-size:1.3rem;">{v_msg}</div><div><span class="status-badge {v_bg}">{vol_r:.1f} 倍均量</span></div><div class="metric-sub">成交量活躍度分析</div></div>""", unsafe_allow_html=True)
                with k3: st.markdown(f"""<div class="metric-card"><div class="metric-title">MACD 趨勢</div><div class="metric-value" style="font-size:1.3rem;">{m_msg}</div><div><span class="status-badge {m_bg}">數值: {last['MACD']:.2f}</span></div><div class="metric-sub">籌碼動能方向</div></div>""", unsafe_allow_html=True)
                with k4: st.markdown(f"""<div class="metric-card"><div class="metric-title">RSI 強弱</div><div class="metric-value" style="font-size:1.3rem;">{r_msg}</div><div><span class="status-badge {r_bg}">數值: {r_val:.1f}</span></div><div class="metric-sub">乖離率判斷</div></div>""", unsafe_allow_html=True)
                ...
                range_breaks = [dict(values=all_dates.difference(df_chart.index).strftime("%Y-%m-%d").tolist())]

                st.markdown("<div class='chart-title'>📈 股價走勢 & 均線</div>", unsafe_allow_html=True)
                fig_price = go.Figure()
                fig_price.add_trace(go.Candlestick(x=df_chart.index, open=df_chart['Open'], high=df_chart['High'], low=df_chart['Low'], close=df_chart['Close'], name='K線', showlegend=False, increasing_line_color=COLOR_UP, decreasing_line_color=COLOR_DOWN))
                for m in [5, 20, 60, 120]:
                    col, wid, dash = ('#D500F9', 1, 'solid') if m==5 else ('#FF6D00', 1.5, 'solid') if m==20 else ('#00C853', 1.5, 'solid') if m==60 else ('#78909C', 1.5, 'dot')
                    fig_price.add_trace(go.Scatter(x=df_chart.index, y=df_chart[f'MA_{m}'], line=dict(color=col, width=wid, dash=dash), name=f'MA{m}'))
                fig_price.update_layout(height=400, margin=dict(l=10, r=10, t=10, b=100), template="plotly_white", xaxis_rangeslider_visible=False, xaxis=dict(showgrid=True, gridcolor='#f0f0f0'), yaxis=dict(showgrid=True, gridcolor='#f0f0f0'), font=dict(color='black'))
                ...
                vol_colors = [VOL_EXPLODE if (v/ma >= 2) else VOL_NORMAL if (v/ma >= 1) else VOL_SHRINK for v, ma in zip(df_chart['Volume'], df_chart['Vol_MA'].replace(0, 1))]
                fig_vol = go.Figure()
                ...
                fig_vol.update_layout(height=250, margin=dict(l=10, r=10, t=10, b=10), template="plotly_white", showlegend=False, font=dict(color='black'))
                ...
                fig_rsi.update_layout(height=250, margin=dict(l=10, r=10, t=10, b=10), template="plotly_white", font=dict(color='black'))
                ...
                fig_macd = go.Figure()
                macd_cols = [MACD_BULL_GROW if (h>=0 and h>ph) else MACD_BULL_SHRINK if (h>=0 and h<=ph) else MACD_BEAR_GROW if (h<0 and h<ph) else MACD_BEAR_SHRINK for h, ph in zip(df_chart['Hist'], df['Hist'].shift(1).loc[df_chart.index])]
                ...
                fig_macd.add_trace(go.Bar(x=df_chart.index, y=df_chart['Hist'], marker_color=macd_cols, name='Hist'))
                fig_macd.update_layout(height=250, margin=dict(l=10, r=10, t=10, b=10), template="plotly_white", font=dict(color='black'))
